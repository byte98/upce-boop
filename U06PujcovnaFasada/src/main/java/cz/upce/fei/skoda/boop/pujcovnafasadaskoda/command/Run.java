...
import cz.upce.fei.skoda.boop.pujcovnafasadaskoda.CommandLine;
...
public class Run extends Command
{
    ...
    public String execute(String[] arguments)
    {
        ...
        PrintStream cout = Configuration.getCommandsOutput();
        try
        {
            try (InputStream inputStream = new FileInputStream(file)) {
                Interpreter interpreter = new Interpreter(this.dataManager, inputStream, Configuration.getCommandsOutput());
                while(interpreter.hasNext())
                {
                    String out = interpreter.read();
                    if ((interpreter.isError() || (this.isVerbose() && out.equals("") == false)) && Configuration.commandsPrint())
                    {
                        cout.println(out.trim());
                    }
                }
                cout.close();
