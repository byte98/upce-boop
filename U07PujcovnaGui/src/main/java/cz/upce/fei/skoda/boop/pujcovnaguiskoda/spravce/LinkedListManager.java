 ...
 */
//</editor-fold>
package cz.upce.fei.skoda.boop.pujcovnaguiskoda.spravce;

import cz.upce.fei.skoda.boop.pujcovnaguiskoda.generator.BusGenerator;
import cz.upce.fei.skoda.boop.pujcovnaguiskoda.generator.Generator;
import cz.upce.fei.skoda.boop.pujcovnaguiskoda.generator.IGenerator;
import cz.upce.fei.skoda.boop.pujcovnaguiskoda.generator.TrainGenerator;
...
import cz.upce.fei.skoda.boop.pujcovnaguiskoda.perzistence.BinaryReader;
import cz.upce.fei.skoda.boop.pujcovnaguiskoda.perzistence.BinaryWriter;
...
public class LinkedListManager implements IManager
{
    ...
    {
        boolean reti = true;
        Mappable previous = this.getPrevious();
        if (Objects.nonNull(previous))
        {
            this.setAcutal(previous);
        }
        return reti;
    }
    ...
    /**
     * Gets previous element to actual one
     * @return Previous element to actual one
     *         or NULL if there is no such an element
     */
    private Mappable getPrevious()
    {
        Mappable reti = null;
        if (Objects.nonNull(this.getActual()))
        {
            Iterator<Mappable> it = this.data.iterator();
            while (it.hasNext())
            {
                Mappable next = it.next();
                if (next.equals(this.getActual()))
                {
                    break;
                }
                else
